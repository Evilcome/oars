...
function bind( /* path, target, verb, options */ ) {
  ...
  // of the keys as route options and then bind the target.
    ...
    // accordingly.  This makes it easier to add functionality to Oars.
    oars.emit('route:typeUnknown', {
      path: path,
      target: target,
      verb: verb,
      options: options
...
function bindRedirect(path, redirectTo, verb, options) {
  ...
  bind.apply(this,[path, function(req, res) {
    oars.log.verbose('Redirecting request (`' + path + '`) to `' + redirectTo + '`...');
...
function bindArray(path, target, verb, options) {
  ...
  if (target.length === 0) {
    oars.log.verbose('Ignoring empty array in `router.bind(' + path + ')`...');
  } else {
...
function bindFunction(path, fn, verb, options) {
  ...
  oars.log.silly('Binding route :: ', verb || '', path, _middlewareType?('('+_middlewareType+')'):'');
  ...
  var skipRegexesWrapper = function(regexes, fn) {
    ...
    regexes = oars.util.compact(regexes.map(function(regex) {
      if (regex instanceof RegExp) {
        return regex;
      }
      oars.log.warn('Invalid regex "' + regex + "' supplied to skipRegexesWrapper; ignoring.");
      return undefined;
    }));
  ...
  targetVerb.forEach(function(verb) {
    ...
    oars.router._privateRouter[verb](path, targetFn);
    ...
    // oars.hooks.http.app[verb || 'all'](path, target);
    oars.emit('router:bind', {
      path: path,
      target: _.clone(targetFn),
      verb: verb,
      originalFn: fn
    });
...
/**
 * Sanitize the arguments to `oars.router.bind()`
 *
...
function sanitize(path, target, verb, options) {
  ...
  var detectedVerb = _.detectVerb(path);
