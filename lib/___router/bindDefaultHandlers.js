  ...
  return {
    ...
    500: function(err, req, res) {
      ...
      // it should monitor for `abort` events and manage its private resources (e.g. TCP sockets) accordingly.
        ...
        oars.log.error('But no response could be sent because another error occurred:');
        oars.log.error(errorSendingResponse);
      }
    },
    ...
    404: function(req, res) {
      ...
      // Log a message and try to use `res.send` to respond.
      try {
        oars.log.verbose('A request did not match any routes, and no `res.notFound` handler is configured.');
        res.send(404);
        return;
      }
      ...
      // it should monitor for `abort` events and manage its private resources (e.g. TCP sockets) accordingly.
        ...
        oars.log.error('An unmatched route was encountered in a request...');
        oars.log.error('But no response could be sent because an error occurred:');
        oars.log.error(e);
        return;
      }
    }
