module.exports = function(oars) {
	...
	var policyHookDef = {
		...
		initialize: function(cb) {
			...
			this.loadMiddleware( function (err) {
				...
				oars.log.verbose('Finished loading policy middleware logic.');
				cb();
			});

			// Before routing, curry controller functions with appropriate policy chains
			oars.on('router:before', this.bindPolicies);
		},
		...
		loadMiddleware: function(cb) {
			...
			// Load policy modules
			oars.log.verbose('Loading policy modules from app...');
			oars.modules.loadPolicies(function modulesLoaded (err, modules) {
				if (err) return cb(err);
		...
		bindPolicies: function() {
			...
			// oars.log.verbose('Binding policies :: \n', this.mapping,
			// 	'\nto controllers :: \n', oars.middleware.controllers);

			// Policies can be bound to:
			//  -> controllers
			_bindPolicies(this.mapping, oars.middleware.controllers);
			...
			oars.log.verbose('Policy-controller bindings complete!');
			oars.emit('hook:policies:bound');
		 ...
		 * by performing a non-destructive parse of `oars.config.policies`
		 *
		...
		buildPolicyMap: function () {
			...
			var mapping = {};
			util.each(oars.config.policies, function (_policy, controllerId) {
        ...
        alwaysAllow._middlewareType  = 'POLICY: alwaysAllow';
			...
			oars.log.error('Cannot map invalid policy: ', policy);
		...
		bindDirectlyToRoute: function (event) {
			...
			oars.router.bind(event.path, fn, event.verb);
		},
		...
		lookupFn: function (policyId, referencedIn) {
			...
			if ( !this.middleware[policyId] ) {
				return Err.fatal.__UnknownPolicy__ (policyId, referencedIn, oars.config.paths.policies);
			}
	 ...
	 */
	oars.on('route:typeUnknown', function (ev) {
	...
	function _bindPolicies ( mapping, middlewareSet ) {
		util.each(middlewareSet, function (_c, id) {
			...
			// oars.log.verbose('Applying policy :: ', topLevelPolicyId, ' to ', id);
			...
			if ( !util.isDictionary(topLevelPolicyId) ) {
				...
				if ( util.isDictionary(controller) ) {
					// oars.log.verbose('Applying policy (' + topLevelPolicyId + ') to controller\'s (' + id + ') actions...');
					util.each(actions, function(actionId) {
						actionFn = controller[actionId];
						controller[actionId] = topLevelPolicyId.concat([actionFn]);
						// oars.log.verbose('Applying policy to ' + id + '.' + actionId + '...', controller[actionId]);
					}, this);
					return;
				}
				...
				// oars.log.verbose('Applying policy (' + topLevelPolicyId + ') to top-level controller middleware fn (' + id + ')...');
			...
			// iterate through each of this controller's actions and apply policies in a way that makes sense
				...
				util.each(actions, function(actionId) {
					...
					// oars.log.verbose('Mapping policies to actions.... ', actions);
					...
					// oars.log.verbose('Applying policy (' + actionPolicy + ') to action (' + id + '.' + actionId + ')...');
