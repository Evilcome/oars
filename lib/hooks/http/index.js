...
module.exports = function(oars) {
  ...
  var initialize = require('./initialize')(oars);
  ...
  return {
    ...
    defaults: function(config) {
      return {
        ...
        //  • an absolute path
        //  • a relative path from the app root (oars.config.appPath)
        paths: {
          public: '.tmp/public'
        },
    ...
    configure: function() {
      ...
      if (oars.config.ssl && (
        oars.config.ssl.cert && !oars.config.ssl.key
      ) || (!oars.config.ssl.cert && oars.config.ssl.key)) {
        throw new Error('Invalid SSL config object!  Must include cert and key!');
      }

      if (oars.config.host) {
        oars.config.explicitHost = oars.config.host;
      }

      if (oars.config.express) {
        oars.log.warn('`oars.config.express` is deprecated; use `oars.config.http` instead.');
      }
      ...
      //  • an absolute path
      //  • a relative path from the app root (oars.config.appPath)
      oars.config.paths.public = path.resolve(oars.config.appPath, oars.config.paths.public);

      // Merge in legacy `oars.config.express` object for backwards-compat.
      oars.util.defaultsDeep(oars.config.http, oars.config.express||{});
      ...
      oars.config.http.middleware.order = oars.config.http.middleware.order || oars.hooks.http.defaults(oars.config).http.middleware.order;
    ...
    initialize: function(cb) {
      ...
      if ( oars.hooks.session ) {
        oars.after('hook:session:loaded', function () {
      ...
      // If the session hook is not available:
        ...
        _.remove(oars.config.http.middleware.order, function (mwr){
