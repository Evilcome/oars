...
module.exports = function(oars, app) {
  ...
  return _.defaults(oars.config.http.middleware || {}, {

    // Configure flat file server to serve static files
    // (By default, all explicit+shadow routes take precedence over flat files)
    www: (function() {
      var flatFileMiddleware = express['static'](oars.config.paths['public'], {
        maxAge: oars.config.cache.maxAge
      });
    ...
    session: (function() {
      if (oars.config.session && oars.config.session.store) {
        return express.session(oars.config.session);
      }
    })(),
    ...
    cookieParser: (function() {
      ...
      var cookieParser =
        oars.config.http.cookieParser || oars.config.http.middleware.cookieParser;
      var sessionSecret = oars.config.session && oars.config.session.secret;
    ...
    bodyParser: (function() {
      ...
      if (oars.config.http.bodyParser) {
        fn = oars.config.http.bodyParser;
        return fn(opts);
      } else if (oars.config.http.bodyParser === false) {
      ...
      ////////////////////////////////////////////////////////
      var conf = oars.config.http.middleware.bodyParser;
      if (conf) {
        ...
        // middleware function defined directly
          ...
          try {
            ...
            oars.on('lifted', function () {
              oars.log.error(e);
              oars.log.blank();
              oars.log.error('Encountered an error when trying to use configured bodyParser.');
              oars.log.error('Usually, this means that it was installed incorrectly.');
              oars.log.error('A custom bodyParser can be configured without calling the wrapper function- e.g.:\n'+
                '```\n'+
        ...
        // invalid conf - warn and ignore
          ...
          // TODO: pull this out into a more generic config validator for Oars in general
          oars.log.warn(
    ...
    handleBodyParserError: function handleBodyParserError(err, req, res, next) {
      ...
      oars.log.error(bodyParserFailureErrorMsg);
    ...
    methodOverride: (function() {
      if (oars.config.http.methodOverride) {
        return oars.config.http.methodOverride();
      }
    })(),
    ...
    // Add powered-by Oars header
    poweredBy: function xPoweredBy(req, res, next) {
      res.header('X-Powered-By', 'Oars <oarsjs.org>');
      next();
    },
