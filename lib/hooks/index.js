...
module.exports = function(oars) {
  ...
  function Hook(definition) {
    ...
    this.load = function(cb) {
      ...
      // Determine if this hook should load based on Oars environment & hook config
      if (this.config.envs &&
        this.config.envs.length > 0 &&
        this.config.envs.indexOf(oars.config.environment) === -1) {
        return cb();
      }

      // Convenience config to bind routes before any of the static app routes
      oars.on('router:before', function() {
        _.each(self.routes.before, function(middleware, route) {
          ...
          oars.router.bind(route, middleware);
        });
      });

      // Convenience config to bind routes after the static app routes
      oars.on('router:after', function() {
      ...
      /**
       * Apply hook default configuration to oars.config
       *
       * @api private
       */
      function applyDefaults() {
        defaultsDeep(oars.config,
          _.isFunction(self.defaults) ?
          self.defaults(oars.config) :
          self.defaults);
      }
      ...
      async.auto({
        ...
        configuration: function(cb) {
          ...
          if (oars.config.hooks.userconfig) {
            ...
            oars.after('hook:userconfig:loaded', function() {
              applyDefaults();
              self.configure();
              return cb();
            });
        ...
        modules: ['configuration',
          function(cb) {
            ...
            if (oars.config.hooks.moduleloader) {

              // Load modules
              oars.after('hook:moduleloader:loaded', function moduleloaderReady() {
     ...
     * Then fold modifications back into `oars.config`
     *
