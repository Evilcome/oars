...
module.exports = function (oars) {
	...
	function interpretRouteSyntax (route) {
		...
		if (_.isObject(target) && !_.isFunction(target) && !_.isArray(target)) {
			...
			if ( !options.detectedVerb ) {
				if ( target.get ) { oars.router.bind (path, target['get'],'get', options); }
				if ( target.post ) { oars.router.bind (path, target['post'],'post', options); }
				if ( target.put ) { oars.router.bind (path, target['put'],'put', options); }
				if ( target['delete'] ) { oars.router.bind (path, target['delete'],'delete', options); }
		...
		if (_.isString(target)) {
			...
			if ( controllerId && (
				oars.middleware.controllers[controllerId] ||
				(oars.config.hooks.views.blueprints && oars.middleware.views[controllerId])
				)
			) {
	...
	function bindController ( path, target, verb, options ) {
		...
		var controller = oars.middleware.controllers[controllerId];

		// Fall back to matching view
		if (!controller) {
			controller = oars.middleware.views[controllerId];
		}
    ...
    // so just double-check to see if the attached model exists in `oars.models`.
    if (oars.hooks.orm && oars.models && oars.models[modelId]) {

      // If a model with matching identity exists,
      // extend route options with the id of the model.
      options.model = modelId;

      var Model = oars.models[modelId];
      ...
      options = _.defaults(options, {
        populate: oars.config.blueprints.populate,
        defaultLimit: oars.config.blueprints.defaultLimit
      });
		...
		if (_.isArray(subTarget)) {
			_.each(subTarget, function bindEachMiddlewareInSubTarget (fn) {
				oars.router.bind(path, controllerHandler(fn), verb, options);
			});
			return;
		}
		...
		oars.router.bind(path, controllerHandler(subTarget), verb, options);
	...
	function bindBlueprintAction (path, blueprintActionID, verb, options){
		...
		var blueprint = oars.middleware.blueprints[blueprintActionID];
		...
		oars.router.bind(path, blueprint, verb, options);
	}
