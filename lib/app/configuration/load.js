...
module.exports = function(oars) {
  ...
  return function loadConfig(cb) {
    ...
    async.auto({
         ...
         * Oars on the `oars` object (from its `package.json`)
         */
        versionAndDependencyInfo: function(cb) {

          var pathToThisVersionOfOars = path.join(__dirname, '../../..');
          oars.util.getPackage(pathToThisVersionOfOars, function(err, json) {
            if (err) return cb(err);

            oars.version = json.version;
            oars.majorVersion = oars.version.split('.')[0].replace(/[^0-9]/g, '');
            oars.minorVersion = oars.version.split('.')[1].replace(/[^0-9]/g, '');
            oars.patchVersion = oars.version.split('.')[2].replace(/[^0-9]/g, '');
            oars.dependencies = json.dependencies;
