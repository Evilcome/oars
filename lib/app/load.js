...
module.exports = function(oars) {
  ...
  var Configuration = __Configuration(oars);
  var initializeHooks = __initializeHooks(oars);
  ...
  return function load(configOverride, cb) {
    ...
    // user config loads.
    oars.on('hook:userconfig:loaded', oars.exposeGlobals);
    ...
    async.auto({
      ...
      registry: ['hooks',
        function populateRegistry(cb) {
          ...
          oars.emit('middleware:registered');
