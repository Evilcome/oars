  ...
  return function initializeHooks(hooks, cb) {
    ...
    _.each(hooks, function(hookPrototype, id) {
      ...
      if (!_.isFunction(hookPrototype)) {
        oars.log.error('Malformed hook! (' + id + ')');
        oars.log.error('Hooks should be a function with one argument (`oars`)');
        process.exit(1);
      }

      // Instantiate the hook
      var def = hookPrototype(oars);
    ...
    async.auto({
        ...
        initialize: function(cb) {
          async.each(_.keys(hooks), function initializeHook(id, cb) {
            oars.log.silly('Loading hook: ' + id);
            hooks[id].load(function(err) {
              if (err) {
                oars.log.error('A hook (`' + id + '`) failed to load!');
                oars.emit('hook:' + id + ':error');
                return cb(err);
              }

              oars.log.verbose(id, 'hook loaded successfully.');
              oars.emit('hook:' + id + ':loaded');
